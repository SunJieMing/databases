...
describe("Persistent Node Chat Server", function() {
  ...
  it("Should insert posted messages to the DB", function(done) {
    ...
    request({method: "POST",
            ...
            function () {
              ...
              var queryString = "SELECT message FROM users, messages WHERE users.id = messages.userid AND users.username = 'Valjean';";
              ...
              dbConnection.query( queryString, queryArgs,
                function(err, results) {
                  console.log('test', 'err ', err, 'res ', results[0].message)
  ...
  it("Should output all messages from the DB", function(done) {
    ...
    var queryString = "INSERT into messages(message, userid) values ('Men like you can never change!', 1); "

    dbConnection.query(queryString, function(err, res){
      done();
    })

    var queryString = "INSERT into users(id, username) values(1, 'b');";
    var queryArgs = [];
      ...
      function(err) {
        ...
        request("http://127.0.0.1:3000/classes/messages",
          function(error, response, body) {
            if (err) {console.log('final test', error)}
              console.log('final test', body)
